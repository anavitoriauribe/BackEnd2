...
**Tema:** Camda de acesso a dados, driver PDO(PHP Data Objects), Driver `pdo_pgsql`, padrão singleton, isolamento de credenciaisi (.env) e tratamento de exceções (PDepexception).
...
Faz a conversão de php - sql, sql - php
...
$opcoes = [
    ...
    //3. Desativa emulação e utiliza prepared statements nativos 
...
- PDO::ATTR_DEFAULT_FETCH_MODE => PDO::FETCH_ASSOC : por padrão o método `fetch()`retorna um array duplicado contendo índices numéricos`[0,1]`e associativos`["id","código_maquina"]`. Definir `FETCH_ASSOC`reduz o consumo de memória RAM pela metade e entrega coleções limpas.
...
2. Robôs maliciosos varrem repositórios à procura de credenciais expostas para invadir banco de dados e sequestrar informações (ataques de Ransoware)
...
Imagine uma aplicação web cm 500 usuários acessando simultanemante. Se cada script, função executar `new PDO()` sempre que precisar cosultar o banco, teremos milhares de conexões de redes abertas desnecessariamente.
...
* **Para a Equipe de Desenvolvimento**: Gravar os detalhes técnicos completos com timestamp em um arquivo de log seguro (`logs/)
