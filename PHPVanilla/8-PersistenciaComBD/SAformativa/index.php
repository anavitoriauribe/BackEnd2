...
//buscar o caminho dos arquivo de configuração e de log
...
function registrarErroLog(string $mensagem):void {
    $pasta = dirname(CAMINHO_LOG); //busca o arquvio de log
...
function listarLivros(PDO $pdo): array{
    $sql = "SELECT id, titulo, autor, preco, status, data_cadastro FROM livros
    ORDER BY id DESC";
    // criação do statement para realizar a consulta
    $stmt = $pdo->query($sql);
    //retorna a informaçoes em um array
...
//criar a conexão com o pdo usando tratamento de erro (try-cat)
try {
...
$livros=listarLivros($pdo);
    ...
    $mensagemErro ="Não foi possivel se oncetar com o banco de daos. Tente novamente mais tarde.";
} catch (\Throwable $th) {
    registrarErroLog("Erro Geral: ".$th->getMessage());//mensagem para equipe de dev
    $mensagemErro="Ocorreu um erro inesperado do sistema.";//mensagem para o usuario
    //throw $th;
}
?>
