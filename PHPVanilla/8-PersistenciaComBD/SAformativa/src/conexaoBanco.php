...
final class ConexaoBanco{
    ...
    // muda de acordo com a conexao. inicialmente é nulo , mas depois pode mudar para conectado

    //métodos ( ações que o objeto pode fazer)

    // construtor
    private function __construct(){} //construtor vazio e privado
    //construtor vazio e privado( técnica do singleton)
    ...
    public static function obterConexao(string $caminhoConfig):PDO {
        //so vou criar uma nova conexaão se nao existir outra
        if(self::$instancia === null){
            //vou criar uma conexao
    ...
    private static function carregarArquivoConfig(string $caminho):array{
        ...
        if($dados === false || !isset ($dados["database"])){
            throw new \RuntimeException("Seção [database] ausente no arquvio de configuração");
